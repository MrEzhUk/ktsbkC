...
public class UserIdArgumentType implements ArgumentType<Long> {
    String param;
    String rawArgument;
    BiMap<Long, String> loaded;

    public UserIdArgumentType(String param) {
        this.param = param;
        rawArgument = "";
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        ...
        if(rawArgument.startsWith(".")) {
            try {
                ...
                if(sug == null) {
                    WithKbErr<KtsUser> A = null;
                    try {
                        A = service.getUserById(KtsBkApiProvider.INSTANCE.auth(), a);
        ...
        WithKbErr<List<KtsUser>> accounts = null;
        try {
            accounts = service.autoCompleteUser(KtsBkApiProvider.INSTANCE.auth(), getRawArgument());
        ...
        for(KtsUser acc : accounts.u) {
            loaded.forcePut(acc.getId(), acc.getNickname());
            builder.suggest(acc.getNickname());
        }
        return builder.buildFuture();
    }
